...
Architecture:
  ...
  2. Asks user for shop name, phone, email, address
  3. Sequential calls — each file saved immediately after generation
     Architecture → index.html → style.css → script.js
  4. Cleans all <think> tags and markdown fences from every response
"""
...
async def generate_and_save_architecture(project_idea: str, details: str, out: str) -> str:
    system = AGENT_PERSONA + """

Write a DETAILED architecture document in Markdown with EXACTLY these sections.
Each section must be thorough — this is the single source of truth for all three files.
...
- **Header/Nav**: logo text, nav links (list all), hamburger on mobile, sticky/fixed
...
- **Contact/Footer section**: must include the shop's phone, email, and address
- All class names to use (must match CSS and JS exactly)

## CSS Specifications
- Layout: flexbox or grid per section, column counts
...
- Use the REAL shop details provided — not Lorem Ipsum
- Class names and IDs must be consistent across all three files
"""
    user = (
        f"Project: {project_idea}\n\n"
        f"{details}\n\n"
        "Write the full detailed architecture. Every design decision must be documented "
        "so HTML, CSS, and JS can be built without guessing anything."
    )
...
async def generate_and_save_html(project_idea: str, arch: str, details: str, out: str) -> str:
    system = AGENT_PERSONA + """
...
- Lowercase hyphenated class names matching the architecture exactly
- 150–220 lines, well-commented
- NO inline styles or inline scripts
- Use the REAL shop details (name, phone, email, address) provided — not placeholders
"""
    user = (
        f"Project: {project_idea}\n\n"
        f"{details}\n\n"
...
async def main():
    ...
    if not API_KEY:
        ...
        print("    Add to your .env file:  SARVAM_API_KEY=sk_xxxx\n")
        sys.exit(1)
    ...
    # ── Shop details ─────────────────────────────────────────────────────────
    print("\n   📋  Apni shop ki details do (website mein use hongi):\n")
    shop_name = input("   🏪  Shop ka naam   : ").strip()
    phone     = input("   📞  Contact number : ").strip()
    email     = input("   📧  Email ID       : ").strip()
    address   = input("   📍  Address        : ").strip()

    # Fallbacks if user leaves blank
    shop_name = shop_name or "My Shop"
    phone     = phone     or "+91 00000 00000"
    email     = email     or "contact@myshop.com"
    address   = address   or "India"

    # Details block injected into every prompt
    details = (
        f"Shop Details (use these EXACTLY in the website — no placeholders):\n"
    ...
    # Output folder named after shop
    folder = re.sub(r"[^a-zA-Z0-9_-]", "_", shop_name[:40]).strip("_") or "my_site"
    ...
    arch = await generate_and_save_architecture(project_idea, details, out)

    # ── Phase 2: HTML ────────────────────────────────────────────────────────
    print("\n[PHASE 2/4] HTML …")
    await generate_and_save_html(project_idea, arch, details, out)
